...
<div class="comments-block">
    <div class="comments-block__content loadJs">
        ...
        <? } else { ?>
            <form class="comments-block__form commentFormJs captchaFormJs" data-goal="make_order"
                  action="<?= $arParams["PAGE_URL"] ?>"
                  method="post"
                  <? if (!empty($_REQUEST['ACTION']) && $_REQUEST['ACTION'] == 'addComment'){ ?>style="display: block"<? } ?>>
            ...
            <? if ($arResult['ADD_COMMENT_AVAL']) { ?>
                <div class="comments-block__btn addCommentJs"
                     <? if (!empty($_REQUEST['ACTION']) && $_REQUEST['ACTION'] == 'addComment'){ ?>style="display: none"<? } ?>><?= Loc::getMessage('DX_CMT_FORM_ADD') ?></div>
            <? } ?>
        <? } ?>
    </div>
</div>
